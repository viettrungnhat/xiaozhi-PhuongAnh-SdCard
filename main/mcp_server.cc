...
void McpServer::AddCommonTools() {
    ...
    auto music = Application::GetInstance().GetMusic();
     if (music) {
         AddTool("self.music.play_song",
				 ...
				 "\n"
				 "⚠️ ĐÓI VỚI YÊU CẦU SD CARD/OFFLINE:\n"
				 "Nếu người dùng nói: 'nhạc trong thẻ nhớ', 'nhạc offline', 'bài trong thẻ', "
				 "'SD card', 'chạy nhạc nội bộ', 'local music' - KHÔNG dùng tool này!\n"
				 "Thay vào đó, hãy dùng tool `self.sdmusic.playback` với action='play'.\n"
				 "\n"
				 "Args:\n"
                 ...
                 [music](const PropertyList &properties) -> ReturnValue {
                     ...
                     // Check if user is asking for SD card music
                     std::string name_lower = song_name;
                     std::transform(name_lower.begin(), name_lower.end(), name_lower.begin(), ::tolower);
                     
                     if (name_lower.find("sd card") != std::string::npos ||
                         name_lower.find("thẻ nhớ") != std::string::npos ||
                         name_lower.find("offline") != std::string::npos ||
                         name_lower.find("local") != std::string::npos ||
                         name_lower.find("thẻ") != std::string::npos) {
                         return "{\"success\": false, \"message\": \"Để phát nhạc từ thẻ nhớ, vui lòng dùng tool self.sdmusic.playback với action=play. Nhạc trong thẻ SD đã được hỗ trợ!\"}";
                     }
	...
	if (sd_music) {
		...
		AddTool("self.sdmusic.playback",
				"🎵 PHÁT NHẠC TỪ THẺ NHỚ (SD card). DÙNG KHI NGƯỜI DÙNG NÓI VỀ NHẠC OFFLINE/LOCAL.\n"
				"\n"
				"⚠️ KÍCH HOẠT KHI:\n"
				"- Người dùng nói 'phát nhạc từ thẻ nhớ', 'nhạc offline', 'SD card', 'nhạc local', 'nhạc nội bộ'\n"
				"- Nếu chỉ nói 'phát nhạc', 'mở bài' mà không nói SD card → dùng self.music.play_song (online)\n"
				"\n"
				"action = play | pause | stop | next | prev\n"
				"  play:  Phát bài hát đầu tiên trong thẻ nhớ\n"
				"  pause: Tạm dừng nhạc\n"
				"  stop:  Dừng nhạc\n"
				"  next:  Bài hát tiếp theo\n"
				"  prev:  Bài hát trước đó\n"
				"\n"
				"Return: Trạng thái điều khiển (JSON hoặc true/false).\n",			
			PropertyList({
		...
		// ================== 7) DANH SÁCH VÀ PHÁT THEO SỐ ==================
		// Gộp: list_names và play_number
		AddTool(
			"self.sdmusic.quick",
			"🎵 PHÁT BÀI THEO SỐ THỨ TỰ TRONG THẺ NHỚ.\n"
			"\n"
			"⚠️ BẮT BUỘC DÙNG KHI:\n"
			"- Người dùng nói 'mở bài số 1', 'phát bài 5', 'bài thứ 3 trong thẻ nhớ'\n"
			"- Người dùng nói 'mở bài 1 trong thẻ nhớ', 'phát bài số 2 từ SD card'\n"
			"- Bất kỳ yêu cầu phát bài có SỐ THỨ TỰ + THẺ NHỚ/SD CARD\n"
			"\n"
			"action = list | play_number\n"
			"  list:        lấy danh sách tên bài hát\n"
			"  play_number: PHÁT BÀI THEO SỐ (number = số thứ tự bài, 1 = bài đầu tiên)\n"
			"\n"
			"Ví dụ: 'Mở bài 1 trong thẻ nhớ' → action='play_number', number=1",
			PropertyList({
				Property("action",     kPropertyTypeString),
				Property("max_tracks", kPropertyTypeInteger, 20, 1, 100),
				Property("number",     kPropertyTypeInteger, 1, 1, 10000)
			}),
			[sd_music](const PropertyList& props) -> ReturnValue {
				if (!sd_music) {
					return "{\"success\": false, \"message\": \"SD music module not available\"}";
				...
				if (action == "list") {
					int max_tracks = props["max_tracks"].value<int>();
					if (max_tracks <= 0) max_tracks = 20;
					return sd_music->getTrackListAsString((size_t)max_tracks);
				}

				if (action == "play_number") {
					int number = props["number"].value<int>();
					if (number <= 0) {
						return "{\"success\": false, \"message\": \"Số thứ tự phải lớn hơn 0\"}";
					}

					// Convert 1-based sang 0-based index
					int index = number - 1;
					
					if ((size_t)index >= sd_music->getTotalTracks()) {
						return "{\"success\": false, \"message\": \"Số thứ tự vượt quá số bài hát trong thẻ nhớ\"}";
					}

					bool ok = sd_music->setTrack(index);
					if (ok) {
						auto track = sd_music->getTrackInfo(index);
						std::string name = track.title.empty() ? track.name : track.title;
						std::string msg = "{\"success\": true, \"message\": \"Đang phát bài số " + 
							std::to_string(number) + ": " + name + "\"}";
						return msg;
					}
					return "{\"success\": false, \"message\": \"Không thể phát bài hát\"}";
				}
