...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    // If content is empty, clear all chat messages (conversation ended)
    if(strlen(content) == 0) {
        // Clear all chat bubbles when conversation ends
        lv_obj_clean(content_);
        chat_message_label_ = nullptr;
        // Show the centered AI logo again
        if (emoji_label_ != nullptr) {
...
void LcdDisplay::StopFFT() {
    ...
    if (fft_task_handle != nullptr) {
        ...
        // Wait for the task to stop (wait up to 2 seconds with shorter intervals)
        int wait_count = 0;
        while (fft_task_handle != nullptr && wait_count < 200) {
    ...
    if (music_owner_label_)   lv_obj_add_flag(music_owner_label_, LV_OBJ_FLAG_HIDDEN);
...
void LcdDisplay::periodicUpdateTask() {
        ...
        if (canvas_) {
            // MỌI GỌI LVGL PHẢI DƯỚI LOCK
            {
				...
				if (!(source == DisplaySourceType::NONE && !sd_playing))
				{
					...
					// --- OWNER LINE (DÒNG 3 CỐ ĐỊNH) ---
					lv_obj_t* owner = lv_label_create(music_root_);
					lv_obj_set_style_text_font(owner, &lv_font_montserrat_14, 0);  // Font nhỏ hơn
					lv_obj_set_style_text_color(owner, lv_color_hex(0x00BFFF), 0);  // Màu cyan nổi bật
					lv_label_set_text(owner, "0986183806 PA");
					lv_obj_set_width(owner, canvas_width_);
					lv_obj_set_style_text_align(owner, LV_TEXT_ALIGN_CENTER, 0);  // Căn giữa
					lv_obj_align(owner, LV_ALIGN_BOTTOM_MID, 0, -6);  // Căn giữa dưới cùng
					music_owner_label_ = owner;
    ...
    while (!fft_task_should_stop) {
        ...
        // Check stop flag frequently to allow quick exit
        if (fft_task_should_stop) break;
        
        TickType_t currentTime = xTaskGetTickCount();
        
        // Process audio data at regular intervals
        if (currentTime - lastAudioTime >= audioProcessInterval) {
            if (final_pcm_data_fft != nullptr && !fft_task_should_stop) {
                processAudioData();  // Quick processing, non-blocking
            } else {
                vTaskDelay(pdMS_TO_TICKS(50));  // Reduced delay for faster stop response
            }
            lastAudioTime = currentTime;
        }
        
        // Check stop flag before display refresh
        if (fft_task_should_stop) break;
        
        // Display refresh (30Hz)
        if (currentTime - lastDisplayTime >= displayInterval) {
            if (fft_data_ready && canvas_ && lv_obj_is_valid(canvas_) && !fft_task_should_stop) {
                DisplayLockGuard lock(this);
                if (fft_task_should_stop) break;  // Check again after acquiring lock
                if (canvas_ && lv_obj_is_valid(canvas_)) {  // Double check after lock
                    drawSpectrumIfReady();
        ...
        // Check stop flag before music UI update
        if (fft_task_should_stop) break;
        ...
        if (currentTime - lastClockUpdate >= pdMS_TO_TICKS(1000)) {
            ...
            if (sd && !fft_task_should_stop &&
                music_root_        && lv_obj_is_valid(music_root_) &&
                music_bar_         && lv_obj_is_valid(music_bar_))
            {
                DisplayLockGuard lock(this);
                if (fft_task_should_stop) break;  // Check after acquiring lock
				...
				if (music_next_line_ && lv_obj_is_valid(music_next_line_)) {
					...
					std::string tip = "Tiếp theo 0986183806: " + next_title;
...
void LcdDisplay::DisplayQRCode(const uint8_t* qrcode, const char* text) {
    ...
    ESP_LOGI(TAG, "QR code size: %d, text: %s", qr_size, text != nullptr ? text : "808888809");
